...
class LimeTabularExplainer(object):
    ...
    def __init__(self,
        ...
        #GW-LIME addition: to access the training data in the child prop of the class, line below is added
    ...
    #GW-LIME Addition: Added list_of_prediction_id to tell the predict function the unique ID's of neighborhood points for which we need the prediction for
    def explain_instance(self,
                         data_row,
                        predict_fn ,
                        list_of_prediction_id,
                         labels=(1,),
                         top_labels=None,
                         num_features=10,
                         num_samples=5000,
        ...
        #GW-LIME Addition: Generate predictions for all the points in the neigoborhood of instance of interest
        yss = predict_fn(list_of_prediction_id)      
        
        #GW-LIME Addition: For testing, the code below exports the L2 distances between the instance of interest and the rest of the observations
        distances_export = str(distances.tolist())
        with open('.distances.txt', 'w') as file:
            file.write(distances_export)
        ...
        #GW-LIME Additions: Also return neighborhood weights and predicted_values for experimentation purposes
        for label in labels:
            (ret_exp.intercept[label],
             ret_exp.local_exp[label],
             ret_exp.score, ret_exp.local_pred, ret_exp.weights, ret_exp.predicted_values) = self.base.explain_instance_with_data(
                    scaled_data,
                    yss,
                    distances,
                    label,
...
#GW-LIME Addition: Edited the original Data inverse function, that does not generate synthetic perturbed dataset. Instead, it simply returns all the data points in the original training data EXCEPT the instance that is being explained#########
    ...
    def __data_inverse(self,
        ...
        ##GW-LIME Note: Relevant for the case of the study, we are not discretizing the data
        if self.discretizer is None:
            ...
            ##GW-LIME Addition: Main area of the original function edit. Process: (a) take training data, (b) remove the instance being explained, (c)  return the remaining training dataset, which is the neighborhood around the instance of interest

            preliminary_data = self.training_data
            array_to_remove = data_row
            index_to_remove = None

            for idx, arr in enumerate(preliminary_data):
                if np.array_equal(arr, array_to_remove):
                    index_to_remove = idx
                    break

            if index_to_remove is not None:
                preliminary_data = np.delete(preliminary_data, index_to_remove, axis=0)

            data = preliminary_data #This is the training data after removing the instance of interest
        ...
        #GW-LIME Note: This discretizes the data row that we aim to explain, not relevant for our study
        else:
            first_row = self.discretizer.discretize(data_row)
        
        
        #GW-LIME Addition: For structural requirements, first row of the returned data has to be the instance of interst. Line below ensures that. All the other rows are the neighborhood points, in our cae, the rest of the training data
        data = np.insert(data, 0, data_row, axis = 0)
        inverse = data.copy() 
        
        #GW-LIME Note: Only runs if discretize_continuous is True, not relevant for our case
